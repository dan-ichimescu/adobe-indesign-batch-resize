...
while (!definitionsFile.eof) {
    ...
    if (countLines != 0) {
        ...
        // alert("vizibil_W_mm "+vizibil_W_mm+"\n"+"vizibil_H_mm "+vizibil_H_mm+"\n"+"total_W_mm "+total_W_mm+"\n"+"total_H_mm "+total_H_mm)
        var vizibil_W_mm = vizibil_W_mm.replace(/\,/g, ".");
        var vizibil_H_mm = vizibil_H_mm.replace(/\,/g, ".");        
        var total_W_mm = total_W_mm.replace(/\,/g, ".");
        var total_H_mm = total_H_mm.replace(/\,/g, ".");
...
function idsimargineDocument(vizibil_W, vizibil_H, total_W, total_H, idNumar) {
    ...
    y1 = Number((total_H - vizibil_H) / 2) + Number(vizibil_H) + 2;
    x1 = Number((total_W - vizibil_W) / 2) + Number(2);
    ...
    m_topq = m_top / 2.8346438836889;
    m_leftq = m_left / 2.8346438836889;
...
function aliniereHwDocument(vizibil_W, vizibil_H, total_W, total_H) {
    ...
    if (w > h) { // landscape
        ...
        // var anchor = transformReferencePoint; //app.documents[0]
        // var anchor = app.layoutWindows[0].transformReferencePoint
        //  = AnchorPoint.CENTER_ANCHOR;
    ...
    if (w < h) // portrait
    {
        myHWai.geometricBounds = [myYw + (0.1 * H_hw), myXw + (0.1 * W_hw), h - (0.1 * H_hw), w - (0.1 * W_hw)];
