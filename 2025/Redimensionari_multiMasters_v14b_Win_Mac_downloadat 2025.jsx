...
function aliniereElemente(vizibil_W, vizibil_H, total_W, total_H) {
  ...
  // cu dti:
  // v1
  // var zeceLaSuta = H_vizibil * HwSize + H_vizibil * DtiSize; 
  // v2
  // var zeceLaSuta = H_vizibil * HwSize + (H_vizibil - (H_vizibil * HwSize)) * DtiSize;

  var zeceLaSuta = H_vizibil * 0.1;
  ...
  for (var i = 0; i < myObjectList.length; i++) {
    ...
    var myLabel = myObjectToAling.label;

    var searchString = "douzecilasuta";
    if (contains(myLabel, searchString)) {
      ...
      var boundsofMyObject = getBoundsOfMyObject(myObj);
      ...
      with (myPage) {
        ...
        guides.add(app.activeDocument.layers.itemByName("Image"), {
          orientation: HorizontalOrVertical.horizontal,
    ...
    var searchString0 = "Health_Warning";
    if (contains(myLabel, searchString0)) {
    ...
    var searchString1 = "DeTeIi";
    if (contains(myLabel, searchString1)) {
    ...
    var searchString2 = "linia";

    if (contains(myLabel, searchString2)) {
    ...
    if (contains(mainString, searchString)) {
      var dtipresent = "DeTeIi";
      if (contains(mainString, dtipresent)) {
        var boundsofMyObject = getBoundsOfMyObject(myObjectToAling);
        var moveFrame2 = H_ - (m_bottom + zeceLaSuta + boundsofMyObject[5] + DtiSize); //+ (H_vizibil - H_vizibil * 0.1)
        myObjectToAling.move([boundsofMyObject[1], moveFrame2]);

      } else {
        var boundsofMyObject = getBoundsOfMyObject(myObjectToAling);
        var moveFrame1 = H_ - (m_bottom + zeceLaSuta + boundsofMyObject[5]); //+ (H_vizibil - H_vizibil * 0.1)
        myObjectToAling.move([boundsofMyObject[1], moveFrame1]);
      }
